...
import com.youyi.runner.util.GsonUtil;
import com.youyi.runner.util.OperationLogHelper;
import com.youyi.runner.util.AspectOrdered;
...
@Component
@Aspect
@Order(AspectOrdered.RECORD_OP_LOG.getOrder())
public class RecordOpLogAspect implements ApplicationListener<ApplicationReadyEvent>, Ordered {
    ...
    private static final String RECORD_OP_LOG_THREAD_POOL_CONFIG = "recordOpLogThreadPoolConfig";
    ...
    private void buildOperationLogDO(JoinPoint jp, OperationLogDO operationLogDO, Object result, Throwable exception) {
        ...
        if (exception != null) {
            // 记录异常信息
            extraData.setException(exception.getMessage());
        } else if (result != null) {
            // 记录返回值
            extraData.setResult(result.toString());
        }
    ...
    private void checkConfig(String configKey) {
        // 实现配置检查逻辑
    }
